...
function buildFullWorkoutSequence(workout, includeWarmup = true) {
  ...
  blockNames.forEach(blockName => {
    ...
    // Skip auto-rest if:
    // 1. Current block already contains a rest exercise
    // 2. Next block IS a dedicated Rest block (e.g. Block 3 = "Rest" only)
    ...
    if (blockNumber < totalBlocks && !blockHasRest && !nextBlockIsRest) {
      sequence.push({
        name: "REST",
        duration: 120,
