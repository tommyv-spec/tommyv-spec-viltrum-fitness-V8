 ...
 * Uses composite key "exerciseName|tipoDiPeso" for specificity,
 * falls back to plain exerciseName for backwards compatibility
 * @param {string} exerciseName - Name of exercise
 * @param {string} [tipoDiPeso] - Equipment/weight type (optional)
 * @returns {string|null} Weight or null if not found
 */
export function getExerciseWeight(exerciseName, tipoDiPeso) {
  const weights = getExerciseWeights();
  
  // Try composite key first (specific to equipment type)
  if (tipoDiPeso) {
    const compositeKey = `${exerciseName}|${tipoDiPeso}`;
    if (weights[compositeKey]) return weights[compositeKey];
  }
  
  // Fallback to plain exercise name (backwards compatibility)
