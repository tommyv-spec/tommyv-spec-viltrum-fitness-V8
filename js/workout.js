...
const ELEVEN_AUDIO_MAP = {
  ...
  'fai almeno 1 ripetizioni, destra': 'fai-1plus-ripetizioni',   'fai almeno 1 ripetizioni, sinistra': 'fai-1plus-ripetizioni',
  'fai almeno 3 ripetizioni, destra': 'fai-3plus-ripetizioni',   'fai almeno 3 ripetizioni, sinistra': 'fai-3plus-ripetizioni',
  'fai almeno 5 ripetizioni, destra': 'fai-5plus-ripetizioni',   'fai almeno 5 ripetizioni, sinistra': 'fai-5plus-ripetizioni',
  'fai almeno 6 ripetizioni, destra': 'fai-6plus-ripetizioni',   'fai almeno 6 ripetizioni, sinistra': 'fai-6plus-ripetizioni',
  'fai 8 ripetizioni, destra': 'fai-8-ripetizioni',              'fai 8 ripetizioni, sinistra': 'fai-8-ripetizioni',
  'fai 10 ripetizioni, destra': 'fai-10-ripetizioni',            'fai 10 ripetizioni, sinistra': 'fai-10-ripetizioni',
  'fai 12 ripetizioni, destra': 'fai-12-ripetizioni',            'fai 12 ripetizioni, sinistra': 'fai-12-ripetizioni',
  'fai 15 ripetizioni, destra': 'fai-15-ripetizioni',            'fai 15 ripetizioni, sinistra': 'fai-15-ripetizioni',
  'fai 16 ripetizioni, destra': 'fai-16-ripetizioni',            'fai 16 ripetizioni, sinistra': 'fai-16-ripetizioni',
  'fai 20 ripetizioni, destra': 'fai-20-ripetizioni',            'fai 20 ripetizioni, sinistra': 'fai-20-ripetizioni',
  'fai 30 ripetizioni, destra': 'fai-30-ripetizioni',            'fai 30 ripetizioni, sinistra': 'fai-30-ripetizioni',
  'fai il massimo delle ripetizioni, destra': 'fai-max-ripetizioni', 'fai il massimo delle ripetizioni, sinistra': 'fai-max-ripetizioni',

  // Coach tips
  'tip-usa-massimo-peso':    'tip-usa-massimo-peso',
  'tip-se-fermi-riparti':    'tip-se-fermi-riparti',
  'tip-unico-set':           'tip-unico-set',
  'tip-secondo-round':       'tip-secondo-round',
  'tip-scendi-lento':        'tip-scendi-lento',
  'tip-riscaldamento-forza': 'tip-riscaldamento-forza',
  'tip-recupera-peso':       'tip-recupera-peso',
...
async function playExercise(index, exercises, resumeTime = null) {
  ...
  // All subsequent tips fire via the transition block after the reps announcement
  if (!exercise.isLabel && !exercise.isWarmup && (mode === "eleven" || mode === "voice" || mode === "synth")) {
    ...
    if (index === firstRealIdx) {
      if (mode === "eleven") {
        setTimeout(() => speakEleven("tip-usa-massimo-peso", "it-IT").catch(() => speak("Usa il massimo peso che riesci a gestire", "it-IT").catch(() => {})), 1200);
      } else {
        setTimeout(() => speak("Usa il massimo peso che riesci a gestire", "it-IT").catch(() => {}), 1200);
      }
    }
  }
...
function getCoachTip(upcoming, prevExercise) {
  ...
  const isForza   = (upcoming.totalRounds || 1) === 1 && !upcoming.isWarmup;
  const peso      = (upcoming.tipoDiPeso || '').toLowerCase();
  ...
  // TIP 4: circuit, round 2, first exercise
  if (isCircuit && upcoming.roundNumber === 2 && upcoming.exerciseNumber === 1) {
    return "tip-secondo-round";
  }
  ...
  // TIP 6a: forza block, warm-up sets
  if (isForza && (peso.includes('vuoto') || peso.includes('leggero'))) {
    return "tip-riscaldamento-forza";
  }

  // TIP 6b: forza block, reps dropped vs previous set
  if (isForza && prevExercise && prevExercise.name === upcoming.name) {
    const prevReps = parseInt((prevExercise.reps || '').toString().replace('+', ''));
    const curReps  = parseInt(reps.replace('+', ''));
    if (!isNaN(prevReps) && !isNaN(curReps) && curReps < prevReps) {
      return "tip-recupera-peso";
    }
  }
...
async function startExerciseTimer(initialSeconds, exercise, nextExercise) {
  ...
  interval = setInterval(async () => {
    ...
    if (remaining <= 0) {
      ...
      // "fai X ripetizioni [lato]" → then coach tip (awaited so they don't overlap)
      if (upcoming && !upcoming.isLabel && (mode === "eleven" || mode === "voice" || mode === "synth")) {
        (async () => {
          const TIP_TEXT = {
            ...
            "tip-secondo-round":       "Dal secondo round in poi è più duro",
            "tip-scendi-lento":        "Scendi lento, circa tre secondi",
            "tip-riscaldamento-forza": "Sei in fase di riscaldamento. Quando sei pronto puoi passare avanti",
            "tip-recupera-peso":       "Recupera bene. Il peso aumenta",
          };
          ...
          if (upcoming.reps) {
            ...
            await speak(phrase, "it-IT").catch(() => {});
          }

          // 2. Coach tip — after reps finishes
          const tipKey = getCoachTip(upcoming, exercise);
          if (tipKey) {
            if (mode === "eleven") {
              await speakEleven(tipKey, "it-IT").catch(async () => {
                const text = TIP_TEXT[tipKey];
                if (text) await speak(text, "it-IT").catch(() => {});
              });
            } else {
              const text = TIP_TEXT[tipKey];
              if (text) await speak(text, "it-IT").catch(() => {});
