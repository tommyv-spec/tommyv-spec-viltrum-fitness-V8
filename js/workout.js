...
/**
 * Frontend normalization for tipoDiPeso that may come as "0.6" from stale cache.
 * Converts decimal to percentage and maps exercise name to max name.
 */
function normalizeTipoDiPesoFE(tipoDiPeso, exerciseName) {
  if (!tipoDiPeso) return '';
  const str = tipoDiPeso.toString().trim();
  
  // Already has "XX% SomeName" → return as is
  if (str.match(/^\d+(?:\.\d+)?\s*%\s*.+$/)) return str;
  
  // Bare decimal like "0.6" → convert to "60% MaxName"
  const num = parseFloat(str);
  if (!isNaN(num) && num > 0 && num < 1 && !str.includes('%')) {
    const pct = Math.round(num * 100);
    const maxName = _exerciseToMaxNameFE(exerciseName);
    return maxName ? `${pct}% ${maxName}` : `${pct}%`;
  }
  
  // "60%" without name → add exercise name mapping
  const pctOnly = str.match(/^(\d+(?:\.\d+)?)\s*%$/);
  if (pctOnly && exerciseName) {
    const maxName = _exerciseToMaxNameFE(exerciseName);
...
function _exerciseToMaxNameFE(name) {
  if (!name) return null;
 ...
 */
function resolveEquipmentDisplay(tipoDiPeso, exerciseName) {
  if (!tipoDiPeso) return '';
  const normalized = normalizeTipoDiPesoFE(tipoDiPeso, exerciseName);
  const calc = calculateWeightFromMax(normalized);
  if (calc) {
    return `${calc.kg} kg (${calc.percentage}% ${calc.maxName})`;
  }
  // Clean up raw percentage text
  const pctMatch = normalized.match(/^(\d+(?:\.\d+)?)\s*%\s*(.+)$/i);
  if (pctMatch) {
 ...
 */
function resolveEquipmentCompact(tipoDiPeso, exerciseName) {
  if (!tipoDiPeso) return '';
  const normalized = normalizeTipoDiPesoFE(tipoDiPeso, exerciseName);
  const calc = calculateWeightFromMax(normalized);
  if (calc) {
 ...
 */
function resolveMaterialeDisplay(tipoDiPeso, exerciseName) {
  if (!tipoDiPeso) return '';
  const normalized = normalizeTipoDiPesoFE(tipoDiPeso, exerciseName);
  const calc = calculateWeightFromMax(normalized);
  if (calc) {
...
const ELEVEN_AUDIO_MAP = {
  ...
  // "fai X ripetizioni, Destra/Sinistra" — sided exercises (dedicated audio with side announcement)
  ...
  'fai il massimo delle ripetizioni, sinistra': 'fai-max-ripetizioni-sinistra',

  // Rep counts whose clips were never generated until 2026-08-07. The code has always
  // been able to build these phrases, so every one of them fell through to the synth
  // voice — which is why the instructor seemed to drop out on some exercises and not
  // others. Clips now exist in elevenlabs_muscle/.
  'fai 1 ripetizioni, destra': 'fai-1-ripetizioni-destra',
  'fai 1 ripetizioni, sinistra': 'fai-1-ripetizioni-sinistra',
  'fai 2 ripetizioni': 'fai-2-ripetizioni',
  'fai 2 ripetizioni, destra': 'fai-2-ripetizioni-destra',
  'fai 2 ripetizioni, sinistra': 'fai-2-ripetizioni-sinistra',
  'fai 3 ripetizioni': 'fai-3-ripetizioni',
  'fai 3 ripetizioni, destra': 'fai-3-ripetizioni-destra',
  'fai 3 ripetizioni, sinistra': 'fai-3-ripetizioni-sinistra',
  'fai 4 ripetizioni': 'fai-4-ripetizioni',
  'fai 4 ripetizioni, destra': 'fai-4-ripetizioni-destra',
  'fai 4 ripetizioni, sinistra': 'fai-4-ripetizioni-sinistra',
  'fai 5 ripetizioni': 'fai-5-ripetizioni',
  'fai 5 ripetizioni, destra': 'fai-5-ripetizioni-destra',
  'fai 5 ripetizioni, sinistra': 'fai-5-ripetizioni-sinistra',
  'fai 6 ripetizioni': 'fai-6-ripetizioni',
  'fai 6 ripetizioni, destra': 'fai-6-ripetizioni-destra',
  'fai 6 ripetizioni, sinistra': 'fai-6-ripetizioni-sinistra',
  'fai 25 ripetizioni': 'fai-25-ripetizioni',
  'fai 25 ripetizioni, destra': 'fai-25-ripetizioni-destra',
  'fai 25 ripetizioni, sinistra': 'fai-25-ripetizioni-sinistra',
  'fai 40 ripetizioni': 'fai-40-ripetizioni',
  'fai 40 ripetizioni, destra': 'fai-40-ripetizioni-destra',
  'fai 40 ripetizioni, sinistra': 'fai-40-ripetizioni-sinistra',
  'fai 50 ripetizioni': 'fai-50-ripetizioni',
  'fai 50 ripetizioni, destra': 'fai-50-ripetizioni-destra',
  'fai 50 ripetizioni, sinistra': 'fai-50-ripetizioni-sinistra',
  'fai almeno 8 ripetizioni': 'fai-8plus-ripetizioni',
  'fai almeno 8 ripetizioni, destra': 'fai-8plus-ripetizioni-destra',
  'fai almeno 8 ripetizioni, sinistra': 'fai-8plus-ripetizioni-sinistra',
  'fai almeno 10 ripetizioni': 'fai-10plus-ripetizioni',
  'fai almeno 10 ripetizioni, destra': 'fai-10plus-ripetizioni-destra',
  'fai almeno 10 ripetizioni, sinistra': 'fai-10plus-ripetizioni-sinistra',
  'fai almeno 12 ripetizioni': 'fai-12plus-ripetizioni',
  'fai almeno 12 ripetizioni, destra': 'fai-12plus-ripetizioni-destra',
  'fai almeno 12 ripetizioni, sinistra': 'fai-12plus-ripetizioni-sinistra', 'fai il massimo delle ripetizioni, sinistra': 'fai-max-ripetizioni-sinistra',

  // Coach tips
  'tip-usa-massimo-peso':    'tip-usa-massimo-peso',
  'tip-se-fermi-riparti':    'tip-se-fermi-riparti',
  'tip-unico-set':           'tip-unico-set',
  'tip-scendi-lento':        'tip-scendi-lento',
  'tip-annota-pesi':         'tip-annota-pesi',
...
const ELEVEN_EXERCISE_MAP = {
  ...
  'weighted step up destra': 'weighted-step-up',
  'weighted step up sinistra': 'weighted-step-up',
  'bird dog destra': 'bird-dog',
  'bird dog sinistra': 'bird-dog',
  'bottom up kettlebell arnold press destra': 'bottom-up-kettlebell-arnold-press',
  'bottom up kettlebell arnold press sinistra': 'bottom-up-kettlebell-arnold-press',
  'bottom up kettlebell hold destra': 'bottom-up-kettlebell-hold',
  'bottom up kettlebell hold sinistra': 'bottom-up-kettlebell-hold',
  'bottom up kettlebell press destra': 'bottom-up-kettlebell-press',
  'bottom up kettlebell press sinistra': 'bottom-up-kettlebell-press',
  'bulgarian split squat destra': 'bulgarian-split-squat',
  'bulgarian split squat sinistra': 'bulgarian-split-squat',
  'burpees': 'burpees',
  ...
  'dumbbell rows destra': 'dumbbell-rows',
  'dumbbell rows sinistra': 'dumbbell-rows',
  ...
  'one arm bench press destra': 'one-arm-bench-press',
  'one arm bench press sinistra': 'one-arm-bench-press',
  ...
  'rowing snatch destra': 'rowing-snatch',
  'rowing snatch sinistra': 'rowing-snatch',
  ...
  'side plank reach destra': 'side-plank-reach',
  'side plank reach sinistra': 'side-plank-reach',
  'sigsaw rows': 'sigsaw-rows',
  'single dumbbell overhead lunges destra': 'single-dumbbell-overhead-lunges',
  'single dumbbell overhead lunges sinistra': 'single-dumbbell-overhead-lunges',
  'single dumbbell thruster destra': 'single-dumbbell-thruster',
  'single dumbbell thruster sinistra': 'single-dumbbell-thruster',
  'sit up on bench': 'sit-up-on-bench',
  'squat jump': 'squat-jump',
  'squat pulse': 'squat-pulse',
  'staggered stance deadlift destra': 'staggered-stance-deadlift',
  'staggered stance deadlift sinistra': 'staggered-stance-deadlift',
  ...
  'tricep kick back destra': 'tricep-kick-back',
  'tricep kick back sinistra': 'tricep-kick-back',
  ...
  'windmill destra': 'windmill',
  'windmill sinistra': 'windmill',
  ...
  'kot lunges destra': 'kot-lunges',
  'kot lunges sinistra': 'kot-lunges',
  'walkout': 'walkout',
  ...
  'banded kettlebell press destra': 'banded-kettlebell-press',
  'banded kettlebell press sinistra': 'banded-kettlebell-press',
  'banded kettlebell oh hold destra': 'banded-kettlebell-oh-hold',
  'banded kettlebell oh hold sinistra': 'banded-kettlebell-oh-hold',
  ...
  'banded one hamstring hold destra': 'banded-one-hamstring-hold',
  'banded one hamstring hold sinistra': 'banded-one-hamstring-hold',
  'banded one hamstring curl destra': 'banded-one-hamstring-curl',
  'banded one hamstring curl sinistra': 'banded-one-hamstring-curl',
  'banded one hamstring pump destra': 'banded-one-hamstring-pump',
  'banded one hamstring pump sinistra': 'banded-one-hamstring-pump',
  'doppio dumbbell lunges destra': 'doppio-dumbbell-lunges',
  'doppio dumbbell lunges sinistra': 'doppio-dumbbell-lunges',
  'lateral swing destra': 'lateral-swing',
  'lateral swing sinistra': 'lateral-swing',
  'doppio dumbbell clean & press': 'doppio-dumbbell-clean-and-press',
  'wood chop destra': 'wood-chop',
  'wood chop sinistra': 'wood-chop',
  ...
  'crossbody march sinistra su': 'crossbody-march',
  'crush grip press': 'crush-grip-press',
  'one arm swing & pull destra': 'one-arm-swing-and-pull',
  'one arm swing & pull sinistra': 'one-arm-swing-and-pull',
  'farmer march': 'farmer-march',
  'elevated hip tilt destra': 'elevated-hip-tilt',
  'elevated hip tilt sinistra': 'elevated-hip-tilt',
  'standing upright rows': 'standing-upright-rows',
  'single leg elevated hip extension destra': 'single-leg-elevated-hip-extension',
  'single leg elevated hip extension sinistra': 'single-leg-elevated-hip-extension',
  'alternate bench press': 'alternate-bench-press',
  'tall kneeling alt twist & press': 'tall-kneeling-alt-twist-and-press',
  'rnt reverse lunges destra': 'rnt-reverse-lunges',
  'rnt reverse lunges sinistra': 'rnt-reverse-lunges',
  'one arm kettlebell swing destra': 'one-arm-kettlebell-swing',
  'one arm kettlebell swing sinistra': 'one-arm-kettlebell-swing',
  'filly march destra su': 'filly-march',
  'filly march sinistra su': 'filly-march',
  'copenaghen side plank destra': 'copenaghen-side-plank',
  'copenaghen side plank sinistra': 'copenaghen-side-plank',
  'curl & press destra': 'curl-and-press',
  'curl & press sinistra': 'curl-and-press',
  'crush grip curl hold': 'crush-grip-curl-hold',

  // Typo aliases
  'flutterkiks': 'flutterkicks',        // typo in sheet → reuse existing audio
  'step up alternati': 'step-up',       // alias → reuse existing

  // Barbell lifts (uppercase in sheet)
  ...
  'deadlift gambe tese': 'deadlift-gambe-tese',
  'front squat': 'front-squat',
  'hang power clean': 'hang-power-clean',
  'push press': 'push-press',
  'wall ball': 'wall-ball',
  'pull up': 'pull-up',
  'step up destra': 'step-up',
  'step up sinistra': 'step-up',
  'banded abductor destra': 'banded-abductor',
  'banded abductor sinistra': 'banded-abductor',
  'bb zarker deadlift': 'bb-zarker-deadlift',
  'bb hip thrust': 'bb-hip-thrust',
  'airplane hip open destra': 'airplane-hip-open',
  'airplane hip open sinistra': 'airplane-hip-open',
  'airplane hip close destra': 'airplane-hip-close',
  'airplane hip close sinistra': 'airplane-hip-close',

  // Base names for sided exercises (so stripping Destra/Sinistra always finds a match)
  'weighted step up': 'weighted-step-up',
  'bird dog': 'bird-dog',
  'bottom up kettlebell arnold press': 'bottom-up-kettlebell-arnold-press',
  'bottom up kettlebell hold': 'bottom-up-kettlebell-hold',
  'bottom up kettlebell press': 'bottom-up-kettlebell-press',
  'bulgarian split squat': 'bulgarian-split-squat',
  'dumbbell rows': 'dumbbell-rows',
  'rowing snatch': 'rowing-snatch',
  'side plank reach': 'side-plank-reach',
  'single dumbbell overhead lunges': 'single-dumbbell-overhead-lunges',
  'single dumbbell thruster': 'single-dumbbell-thruster',
  'staggered stance deadlift': 'staggered-stance-deadlift',
  'tricep kick back': 'tricep-kick-back',
  'windmill': 'windmill',
  'kot lunges': 'kot-lunges',
  'banded kettlebell press': 'banded-kettlebell-press',
  'banded kettlebell oh hold': 'banded-kettlebell-oh-hold',
  'banded one hamstring hold': 'banded-one-hamstring-hold',
  'banded one hamstring curl': 'banded-one-hamstring-curl',
  'banded one hamstring pump': 'banded-one-hamstring-pump',
  'doppio dumbbell lunges': 'doppio-dumbbell-lunges',
  'lateral swing': 'lateral-swing',
  'wood chop': 'wood-chop',
  'one arm swing & pull': 'one-arm-swing-and-pull',
  'elevated hip tilt': 'elevated-hip-tilt',
  'single leg elevated hip extension': 'single-leg-elevated-hip-extension',
  'rnt reverse lunges': 'rnt-reverse-lunges',
  'one arm kettlebell swing': 'one-arm-kettlebell-swing',
  'copenaghen side plank': 'copenaghen-side-plank',
  'curl & press': 'curl-and-press',
  'one arm bench press': 'one-arm-bench-press',
  'banded abductor': 'banded-abductor',
  'airplane hip open': 'airplane-hip-open',
  'airplane hip close': 'airplane-hip-close',
  'crossbody march': 'crossbody-march',
  'filly march': 'filly-march',

  // v-audio 2026-06-25: missing exercises from Exercises sheet (49 base files)
  'active samson destra': 'active-samson',
  'active samson sinistra': 'active-samson',
  'active samson': 'active-samson',
  'active spider': 'active-spider',
  'bicep curl': 'bicep-curl',
  'bicep curl destra': 'bicep-curl',
  'bicep curl sinistra': 'bicep-curl',
  'bootstrap': 'bootstrap',
  'calf raises destra': 'calf-raises',
  'calf raises sinistra': 'calf-raises',
  'calf raises': 'calf-raises',
  'cat cow': 'cat-cow',
  'crunches': 'crunches',
  'crush grip curl': 'crush-grip-curl',
  'crush grip rows': 'crush-grip-rows',
  'crush grip rows hold': 'crush-grip-rows-hold',
  'curl & press alternati': 'curl-and-press-alternati',
  'down dog with pedal': 'down-dog-with-pedal',
  'down up': 'down-up',
  'dumbbell clean alternato': 'dumbbell-clean-alternato',
  'dumbbell press alternati': 'dumbbell-press-alternati',
  'foam hip shift destra': 'foam-hip-shift',
  'foam hip shift sinistra': 'foam-hip-shift',
  'foam hip shift': 'foam-hip-shift',
  'frog pump': 'frog-pump',
  'goblet squat': 'goblet-squat',
  'hammer curl': 'hammer-curl',
  'hamstring march': 'hamstring-march',
  'hang snatch alternato': 'hang-snatch-alternato',
  'hip pump': 'hip-pump',
  'hip switch': 'hip-switch',
  'iyt': 'iyt',
  'jumping jacks': 'jumping-jacks',
  'kettebell shift destra': 'kettebell-shift',
  'kettebell shift sinistra': 'kettebell-shift',
  'kettebell shift': 'kettebell-shift',
  'kettlebell clean': 'kettlebell-clean',
  'kettlebell sumo deadlift': 'kettlebell-sumo-deadlift',
  'knuckle dragger': 'knuckle-dragger',
  'milko bench press destra': 'milko-bench-press',
  'milko bench press sinistra': 'milko-bench-press',
  'milko bench press': 'milko-bench-press',
  'one leg deadlift destra': 'one-leg-deadlift',
  'one leg deadlift sinistra': 'one-leg-deadlift',
  'one leg deadlift': 'one-leg-deadlift',
  'overhead lunges destra': 'overhead-lunges',
  'overhead lunges sinistra': 'overhead-lunges',
  'overhead lunges': 'overhead-lunges',
  'overhead tricep extension': 'overhead-tricep-extension',
  'pogo hops': 'pogo-hops',
  'press destra': 'press',
  'press sinistra': 'press',
  'press': 'press',
  'quad pull through': 'quad-pull-through',
  'quad push through': 'quad-push-through',
  'seated snatch alternato': 'seated-snatch-alternato',
  'side bend destra': 'side-bend',
  'side bend sinistra': 'side-bend',
  'side bend': 'side-bend',
  'snatch destra': 'snatch',
  'snatch sinistra': 'snatch',
  'snatch': 'snatch',
  'spanish squat': 'spanish-squat',
  'swing destra': 'swing',
  'swing sinistra': 'swing',
  'swing': 'swing',
  'updog to downdog': 'updog-to-downdog',
  'upright rows': 'upright-rows',
  'weighted frog pump': 'weighted-frog-pump',
  'weighted reverse lunges destra': 'weighted-reverse-lunges',
  'weighted reverse lunges sinistra': 'weighted-reverse-lunges',
  'weighted reverse lunges': 'weighted-reverse-lunges',
  'world greatest destra': 'world-greatest',
  'world greatest sinistra': 'world-greatest',
...
function preloadUpcoming(fromStep = 0, count = 5) {
  const seq = fullWorkoutSequence;
  if (!seq || !seq.length) return;
  for (let i = fromStep; i < Math.min(fromStep + count, seq.length); i++) {
    if (seq[i]?.imageUrl) preloadGif(seq[i].imageUrl);
  }
}
...
let firedTips = new Set();        // Each tip fires at most once per workout
...
let timerFired = new Set(); // Track which seconds already triggered audio (persists across pause/resume)

/**
 * Stop all ongoing audio playback. Called on pause to prevent audio pile-up on resume.
 */
function stopAllAudio() {
  // Stop Cloud TTS
  const ttsEl = document.getElementById("tts-audio");
  if (ttsEl) { try { ttsEl.pause(); ttsEl.currentTime = 0; } catch(e) {} }
  
  // Stop browser speech synthesis
  try { speechSynthesis.cancel(); } catch(e) {}
  
  // Stop beep/transition sounds
  const beep = document.getElementById("beep-sound");
  const trans = document.getElementById("transition-sound");
  if (beep) { try { beep.pause(); beep.currentTime = 0; } catch(e) {} }
  if (trans) { try { trans.pause(); trans.currentTime = 0; } catch(e) {} }
  
  // Stop beppe player
  try { beppePlayer.pause(); beppePlayer.currentTime = 0; } catch(e) {}
...
async function _speakNow(text, lang = "it-IT") {
  ...
  // "eleven" mode: try ElevenLabs pre-recorded audio first, then Cloud TTS, then synth
  if (mode === "eleven") {
    const normalizedText = text.toLowerCase().trim();
  ...
  // "voice" mode: Cloud TTS only, then synth fallback (no ElevenLabs)
  if (mode === "voice") {
    try {
      return await speakCloud(text, lang);
    } catch (err) {
...
function buildFullWorkoutSequence(workout, includeWarmup = true) {
  ...
  blockNames.forEach(blockName => {
    ...
    // --- AUTO REST between blocks ---
    // Skip if:
    // 1. Current block contains a rest exercise
    // 2. Next block IS a dedicated Rest block
    // 3. Both blocks have the same single exercise (forza: overview → working sets)
    const blockHasRest = exercises.some(ex => {
      const n = (ex.name || '').toLowerCase();
      return n.includes('rest') || n.includes('riposo') || n.includes('pausa');
    });

    const nextBlockName = blockNumber < totalBlocks ? blockNames[blockNumber] : null;
    const nextBlockExercises = nextBlockName ? blockGroups[nextBlockName] : [];
    const nextBlockIsRest = nextBlockExercises.length > 0 && nextBlockExercises.every(ex => {
      const n = (ex.name || '').toLowerCase();
      return n.includes('rest') || n.includes('riposo') || n.includes('pausa');
    });

    // Same single exercise in both blocks → no rest (e.g. BP overview → BP sets)
    const curRealEx = [...new Set(exercises.filter(e => {
      const n = (e.name || '').toLowerCase();
      return !n.includes('rest') && !n.includes('riposo') && !n.includes('pausa');
    }).map(e => e.name.toLowerCase().trim()))];
    const nextRealEx = [...new Set(nextBlockExercises.filter(e => {
      const n = (e.name || '').toLowerCase();
      return !n.includes('rest') && !n.includes('riposo') && !n.includes('pausa');
    }).map(e => e.name.toLowerCase().trim()))];
    const sameExercise = curRealEx.length === 1 && nextRealEx.length === 1 && curRealEx[0] === nextRealEx[0];

    if (blockNumber < totalBlocks && !blockHasRest && !nextBlockIsRest && !sameExercise) {
      sequence.push({
        name: "REST",
        duration: 120,
...
async function playExercise(index, exercises, resumeTime = null) {
  ...
  // Reset fired-seconds tracker only for NEW exercises, not resume
  if (resumeTime === null) timerFired = new Set();
  ...
  const nextEquipDisplay = hasNextEquipment ? resolveEquipmentDisplay(nextExercise.tipoDiPeso, nextExercise.name) : '';
  ...
  if (exercise.nextBlockPreview && exercise.nextBlockPreview.length > 0) {
    ...
    exercise.nextBlockPreview.forEach(ex => {
      ...
      groupMap[key].sets.push({
        reps: ex.reps || '',
        tipoDiPeso: ex.tipoDiPeso || '',
        eqp: ex.tipoDiPeso ? resolveEquipmentCompact(ex.tipoDiPeso, ex.name) : '',
...
// ═══════════════════════════════════════════════════════════════
// COACH TIPS — spoken after reps announcement at each step
// ═══════════════════════════════════════════════════════════════
function getCoachTip(upcoming, prevExercise) {
  if (!upcoming || upcoming.isLabel || upcoming.isWarmup) return null;

  const reps      = (upcoming.reps || '').toString().trim();
  const isMax     = reps.toLowerCase() === 'max';
  const isCircuit = (upcoming.totalRounds || 1) > 1;

  // TIP 3: MAX reps
  if (isMax) {
    return "tip-se-fermi-riparti";
  }

  // TIP 1: circuit, round 1, first exercise of block
  if (isCircuit && upcoming.roundNumber === 1 && upcoming.exerciseNumber === 1) {
    return "tip-unico-set";
  }

  // TIP 5: circuit, round 1, all other exercises
  if (isCircuit && upcoming.roundNumber === 1 && upcoming.exerciseNumber > 1) {
    return "tip-scendi-lento";
  }
...
async function startExerciseTimer(initialSeconds, exercise, nextExercise) {
  ...
  // 🔒 dedupe: uses module-level timerFired (persists across pause/resume)
  ...
  interval = setInterval(async () => {
    ...
    // paused? stop all audio, save time & stop ticking
    if (isPaused) {
    ...
    if (remaining !== lastSecond) {
      ...
      once(10, async () => {
        ...
        if (nextExercise) {
          ...
          const nxEquipDisplay = nxHasEqp ? resolveEquipmentDisplay(nextExercise.tipoDiPeso, nextExercise.name) : '';
...
async function loadWorkoutFromPlan(username, workoutInfo) {
  ...
  try {
    ...
    // V8.2: Wait for workout details if still loading in background
    if (!DataPreloader.isWorkoutsReady()) {
      await DataPreloader.waitForWorkouts();
    }
...
function updateWorkoutPreview() {
  ...
  const defaultInstructionsImage = "https://lh3.googleusercontent.com/d/1qZSzbWLN0D8rIJ56E4s8pBi3g3Brfw2v";
  ...
  function consolidateMateriale(exercises) {
    ...
    for (const ex of (exercises || [])) {
      ...
      if (m) {
        ...
        otherSet.add(normSpaces(resolveMaterialeDisplay(tp, ex.name)));
      }
    }
...
// Release wake lock when page is hidden; re-acquire when it becomes visible during a workout
