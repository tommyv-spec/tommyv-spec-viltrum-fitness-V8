...
function getCoachTip(upcoming, prevExercise) {
  ...
  // TIP 3: MAX reps — supplement the reps phrase
  if (isMax) {
    return "se ti fermi, riparti";
  }

  // TIP 1: circuit, round 1, first exercise of block → set structure reminder
  if (isCircuit && upcoming.roundNumber === 1 && upcoming.exerciseNumber === 1) {
    return "un unico set per esercizio, non fermarti a metà";
  }

  // TIP 4: circuit, round 2, first exercise → difficulty warning
  if (isCircuit && upcoming.roundNumber === 2 && upcoming.exerciseNumber === 1) {
    return "dal secondo round in poi è più duro";
  }

  // TIP 5: circuit, round 1, all other exercises → eccentric cue
  if (isCircuit && upcoming.roundNumber === 1 && upcoming.exerciseNumber > 1) {
    return "scendi lento, circa tre secondi";
  }

  // TIP 6a: forza block, warm-up sets (can skip ahead)
  if (isForza && (peso.includes('vuoto') || peso.includes('leggero'))) {
    return "sei in fase di riscaldamento, quando sei pronto puoi passare avanti";
  }

  // TIP 6b: forza block, reps dropped vs previous set → heavy set warning
  if (isForza && prevExercise && prevExercise.name === upcoming.name) {
    ...
    if (!isNaN(prevReps) && !isNaN(curReps) && curReps < prevReps) {
      return "recupera bene, il peso aumenta";
    }
  }
...
async function startExerciseTimer(initialSeconds, exercise, nextExercise) {
  ...
  interval = setInterval(async () => {
    ...
    if (remaining <= 0) {
      ...
      // Coach tip for the upcoming exercise
      if (upcoming && (mode === "eleven" || mode === "voice" || mode === "synth")) {
        const tip = getCoachTip(upcoming, exercise);
        if (tip) speak(tip, "it-IT").catch(() => {});
      }
