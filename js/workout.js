...
async function startExerciseTimer(initialSeconds, exercise, nextExercise) {
  ...
  interval = setInterval(async () => {
    ...
    if (remaining <= 0) {
      ...
      // Coach tip for the upcoming exercise (uses pre-recorded ElevenLabs key)
      if (upcoming && (mode === "eleven" || mode === "voice" || mode === "synth")) {
        const tipKey = getCoachTip(upcoming, exercise);
        if (tipKey) {
          if (mode === "eleven") {
            speakEleven(tipKey, "it-IT").catch(() => speak(tipKey, "it-IT").catch(() => {}));
          } else {
            // voice/synth: map key back to Italian text for TTS
            const TIP_TEXT = {
