...
const CACHE_MAX_AGE_USER = 10 * 60 * 1000;      // 10 min — progress changes
const CACHE_MAX_AGE_WORKOUTS = 2 * 60 * 60 * 1000; // 2 hours — plans rarely change
...
const DataPreloader = {
  ...
  async loadAll(email) {
    ...
    // STEP 1: Try localStorage caches (instant)
    const cachedUser = this._loadCache(CACHE_KEY_USER, CACHE_MAX_AGE_USER, userEmail);
    const cachedWorkouts = this._loadCache(CACHE_KEY_WORKOUTS, CACHE_MAX_AGE_WORKOUTS, userEmail);
    ...
    if (cachedUser) {
      ...
      // Background refresh workout data if stale
      if (!cachedWorkouts) {
    ...
    try {
      ...
      if (freshUser) {
        ...
        throw new Error('Empty response from getUserInfo');
      }
    } catch (err) {
      console.error('❌ DataPreloader: getUserInfo fallito:', err);
      this._cache.isLoading = false;
      throw err;
    }
    ...
    // STEP 5: Kick off heavy workout data in background (doesn't block dashboard)
    if (!cachedWorkouts) {
      this._loadWorkoutsBackground(userEmail);
    }
  ...
  _loadCache(key, maxAge, email) {
    try {
      ...
      if (age > maxAge) {
        console.log(`📦 Cache ${key} scaduta (${Math.round(age/1000)}s)`);
        return null;
      }
